...
def load_links(links_file: str = "pgn_links.txt") -> list:
    """Load all URLs from the links file.

    Args:
        links_file: Path to file containing one URL per line.

    Returns:
        List of URL strings (empty lines and comments ignored).
    """
    path = Path(__file__).parent / links_file
    if not path.exists():
...
def main():
    parser = argparse.ArgumentParser(
        description="Download all PGN files from pgn_links.txt"
    )
    parser.add_argument("--links", type=str, default="pgn_links.txt",
                        help="Path to links file (default: pgn_links.txt)")
